...
const UI = (() => {
    ...
    function renderCommunityStatic(el, state, revealedCount) {
        ...
        // Always render remaining empty slots so the pot display doesn't shift
        // as community cards are dealt throughout the hand.
    ...
    function updatePlayer(state, p, i, ackInfo = null, isLastHand = false) {
        ...
        if (betEl) {
            ...
            if (inFinish && state.showdownResults && !p.eliminated && !revealPending) {
                ...
                betEl.className = 'player-bet-label';
            } else if (p.allIn && !p.eliminated) {
                betEl.innerHTML = `<span class="bet-action-word action-allin">ALL-IN</span>`;
                betEl.className = 'player-bet-label';
            } else {
